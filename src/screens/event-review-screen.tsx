...
import { formatVibeLabel, isVibeOptionSlug, VIBE_OPTIONS, VIBE_SLUGS } from '~/constants/vibes';
...
const ATTEND_AGAIN_OPTIONS = ['yes', 'no', 'maybe'] as const;
...
const normalizeAttendAgain = (value?: string | null): AttendAgainFeeling =>
  ATTEND_AGAIN_OPTIONS.includes(value as AttendAgainFeeling)
    ? (value as AttendAgainFeeling)
    : 'maybe';

const VIBE_ENUM_TUPLE = VIBE_SLUGS as [VibeSlug, ...VibeSlug[]];
      ...
      return (
        <Pressable
          ...
          className={`rounded-2xl border px-4 py-3 ${
            isActive ? 'border-primary bg-primary/20' : 'border-white/10 bg-white/5'
          }`}>
          <Flex direction="row" align="center" gap={3}>
            <Text size="xl">{emoji}</Text>
            <Flex className="flex-1">
              <Text className="text-white" bold={isActive}>
                {title}
              </Text>
...
const VibeChips = ({
  ...
  <Flex direction="row" gap={2} wrap="wrap" className="w-full">
    {VIBE_OPTIONS.map(({ slug, emoji }) => {
...
export function EventReviewContent({ eventId, onClose, onSubmitted }: EventReviewContentProps) {
  ...
  useEffect(() => {
    ...
    if (Array.isArray(rawAttendeeVibes)) {
      rawAttendeeVibes.forEach((entry) => {
        if (entry?.subject_user && isVibeOptionSlug(entry.vibe_slug)) {
      ...
      Object.entries(rawAttendeeVibes).forEach(([userId, vibe]) => {
        if (isVibeOptionSlug(vibe)) {
          attendeeVibes[userId] = vibe;
        }
      });
    }

    const eventVibes = Array.isArray(existingReview.event_vibes)
      ? (existingReview.event_vibes.filter(isVibeOptionSlug) ?? [])
      : [];
    ...
    reset({
      ...
      attendAgain: normalizeAttendAgain(existingReview.attend_again),
      npsScore: existingReview.nps_score ?? 7,
      additionalFeedback:
        existingReview.additional_feedback ??
        existingReview.event_comment ??
  ...
  return (
    <SafeAreaView className="flex flex-1 bg-background-dark" edges={['left', 'right', 'bottom']}>
      <KeyboardAvoidingView
        ...
        <ScrollView
          ...
          <Flex className="px-6 pb-2" style={{ paddingTop: Math.max(insets.top, 16) }}>
          ...
          <Flex gap={8} className="px-5 pb-32 pt-6">
            {step === 1 ? (
              <Flex gap={8}>
                <Flex gap={2}>
                        ...
                        <Flex
                          ...
                          <Controller
                            ...
                            render={({ field: { value, onChange } }) => (
                              <VibeChips
                                current={value as VibeSlug | undefined}
                                onPick={onChange}
                              />
                            )}
                          />
        ...
        <View className="px-5 pt-3" style={{ paddingBottom: Math.max(insets.bottom, 16) }}>
          <Flex direction="row" gap={3}>
            ...
            <Button
              ...
              disabled={
                isPending || (step < TOTAL_STEPS ? !isRequiredComplete : !isRequiredComplete)
              }>
              <Text className="text-white" bold>
