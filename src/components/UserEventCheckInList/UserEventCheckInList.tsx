...
export const UserEventCheckInList = React.forwardRef<React.ComponentRef<typeof View>>(
  function UserEventCheckInList(_props, ref) {
    ...
    const navigation =
      useNavigation<NativeStackNavigationProp<RootStackParamList, 'CheckInIndex'>>();
    ...
    if (eventsLoading) {
      return (
        <Flex ref={ref} flex className="bg-background-dark">
          <FlatList
            horizontal
            data={SKELETON_ITEMS}
    ...
    if (!events?.length) {
      return (
        <SafeAreaView ref={ref} className="h-full bg-background-dark">
...
UserEventCheckInList.displayName = 'UserEventCheckInList';
